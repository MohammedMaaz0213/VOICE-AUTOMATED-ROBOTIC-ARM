...
void setup() {
  Serial.begin(9600);
  pinMode(led, OUTPUT); // sets the ledPin to be an output
  servo1.attach(3);     //Attach servo 1 to pin 3, servo 2 to 5, and so on...
  servo2.attach(5);
  servo3.attach(6);
  servo4.attach(9);
  servo5.attach(11);
  ...
  while (Serial.available()) {
    ...
    if (group == 1) {     
      ...
      if (com == 0x13) { //Move
        ...
        Move = 1;     //Put the arm in to moving mode
        Grab = 0;     //Grab mode is off
        //Now we send the import group 2 commands
        delay(10);
        Serial.write(0xAA);
        Serial.write(0x00);
        delay(100);
    ...
    if (group == 2) {
      if (com == 0x11) { //Left
        //We switch to the next group and activate the left mode
        group = 3;
        left = 1;
        right = 0;
        up = 0;
  ...
  if (on == 1) {
    if (Move == 1) {
      ...
      if (up == 1) {
        ...
        //Weithe the angle
        servo2.write(mot2);  
        Serial.println("Servo 2 moved up");  // Add debugging output    
      }//enf of up
      ...
      if (down == 1) {
        ...
        //Weithe the angle
        servo2.write(mot2);  
        Serial.println("Servo 2 moved down");  // Add debugging output    
      }//enf of down
    }//end of move
  }//end of ON
}//end of the loop
